...
import "./AdminAntScoreRequest.css";

export default function AdminBetsRequest() {
  ...
  async function handleWonBet(bet) {
    ...
    try {
      ...
      showMessage(`✅ ${bet.username} amelipwa ${payout}`);
      fetchBets();
  ...
  async function deleteBet(bet) {
    await deleteDoc(doc(standardDb, "antscore", bet.id));
    fetchBets();
  }
  ...
  const filtered = bets
    ...
    .filter(bet => !bet.result); // 🔥 muhimu

  return (
    <div>

      <h2>Admin Bets (AntScore Only)</h2>
