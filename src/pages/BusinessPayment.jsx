...
export default function BusinessPayment() {
  ...
  const numbersRef = collection(db, "depositNumbers");
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      do {
        newIndex = Math.floor(Math.random() * allNumbers.length);
      } while (newIndex === lastIndex && allNumbers.length > 1);

      lastIndex = newIndex;
      setCurrentNumber(allNumbers[newIndex]);
    }, 30 * 60 * 1000);
...
3 Select 1 Business number
4 Enter ${currentNumber.number}
...
Confirm`;
  ...
  return (
    <div className="business-container">
      ...
      <p className="title">Business Account / Lipa Number</p>

      {currentNumber ? (
     <div className="number-box">
  <p>
    <b>Number:</b> {currentNumber.number}
    <button
      className="copy-btn"
      type="button"
